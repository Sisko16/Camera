...
const listento_dashboard = () => {
    ...
    onChildChanged((dashboard_ref), (data) => {
        ...
        try {
            if (data.val().controlName === process.env.CAMERA_BUTTON_LABEL) {
                process_data(data);
            }
            print_to_console(data)
            send_feedback(data); // Add a delay to send_feedback(data) if the node '/2' is 
                                 // updated to fast for the app to pick up the updates
